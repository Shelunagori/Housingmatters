...
class PHPMailer {
  ...
  /**
   * Sets center wrapping on the body of the message to a given number of
   * characters.
   * @var int
   */
  public $WordWrap          = 0;
  ...
  public function WrapText($message, $length, $qp_mode = false) {
    ...
    for ($i = 0 ;$i < count($line); $i++) {
      ...
      for ($e = 0; $e<count($line_part); $e++) {
        $center = $line_part[$e];
        if ($qp_mode and (strlen($center) > $length)) {
          $space_left = $length - strlen($buf) - 1;
          if ($e != 0) {
            if ($space_left > 20) {
              $len = $space_left;
              if ($is_utf8) {
                $len = $this->UTF8CharBoundary($center, $len);
              } elseif (substr($center, $len - 1, 1) == "=") {
                $len--;
              } elseif (substr($center, $len - 2, 1) == "=") {
                $len -= 2;
              }
              $part = substr($center, 0, $len);
              $center = substr($center, $len);
              $buf .= ' ' . $part;
