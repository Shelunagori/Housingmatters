...
<!-- BEGIN EXAMPLE TABLE PORTLET-->
   ...
   $z=0;
	foreach ($result_help_desk as $collection) 
	{
		...
		$help_desk_service_provider_id=(int)$collection['help_desk']['help_desk_service_provider_id'];
...
?>
		<tr class="odd gradeX" >
			...
			<td>
			  <?php if($help_desk_status==0 and $help_desk_service_provider_id==0){ ?> <span class="label " style="background-color: #d84a38;">Open</span><?php  }?>
		<?php if($help_desk_status==1 and $help_desk_service_provider_id==0){ ?> <span class="label label-success">Close</span><?php  } ?>
		<?php if($help_desk_service_provider_id!=0){ ?> <span class="label label-info " >Assigned</span><?php  }?> 
			</td>
